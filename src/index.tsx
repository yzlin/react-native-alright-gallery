...
export const snapPoint = (
  ...
  const filtered = points.filter((p) => Math.abs(point - p) === minDelta);
...
const ResizableImage = React.memo(
  <T,>({
    ...
    const onStart = () => {
      ...
      offset.x.value += translation.x.value;
      offset.y.value += translation.y.value;
    ...
    const pinchGesture = Gesture.Pinch()
      ...
      .onStart(({ focalX, focalY }: { focalX: number; focalY: number }) => {
        "worklet";
        if (!isActive.value) {
          return;
        }
    ...
    useAnimatedReaction(
      ...
      (ty: number | null, prevTy: number | null | undefined) => {
...
const GalleryComponent = <T,>(
  ...
  useAnimatedReaction(
    () => currentIndex.value,
    (newIndex: number) => scheduleOnRN(changeIndex, newIndex),
  ...
  useImperativeHandle(ref, () => ({
    ...
    reset(animated = false) {
      for (const itemRef of refs.current) {
        itemRef.reset(animated);
      }
    },
  }));
...
const Gallery = React.forwardRef(GalleryComponent) as <T>(
